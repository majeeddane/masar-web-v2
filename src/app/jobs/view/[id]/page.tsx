                ...
                <div className="bg-white rounded-[2.5rem] shadow-xl border border-slate-100"> {/* ⚠️ تم حذف overflow-hidden للسماح للقائمة بالظهور */}
                    ...
                    <div className="p-8 md:p-12">
                        <div className="flex flex-col md:flex-row justify-between items-start gap-6 mb-10">
                            <div className="flex items-center gap-5">
                                <div className="w-20 h-20 bg-slate-50 rounded-2xl border border-slate-100 flex items-center justify-center overflow-hidden">
                                    {job.profiles?.avatar_url ? <img src={job.profiles.avatar_url} className="w-full h-full object-cover" /> : <Building2 className="w-10 h-10 text-emerald-600" />}
                                </div>
                                <div>
                                    <div className="flex flex-wrap items-center gap-2 mb-2">
                                        ...
                                        <span className="bg-emerald-100 text-emerald-800 text-xs font-bold px-2.5 py-1 rounded-full border border-emerald-200">
                                            {job.category || 'وظيفة معتمدة'}
                                        </span>
                                    </div>
                                    <p className="text-[#115d9a] font-bold flex flex-wrap items-center gap-2 text-sm">
                                        <span>{job.company_name || job.company || job.profiles?.full_name || 'جهة معتمدة'}</span>
                                        <span className="text-slate-300">•</span>
                                        <span className="text-slate-500">{job.city || job.location || 'العمل عن بعد'}</span>
                                        {job.job_type && (
                                            <>
                                                <span className="text-slate-300">•</span>
                                                <span className="text-emerald-600 font-semibold">{job.job_type}</span>
                                            </>
                                        )}
                                        {job.salary_range && job.salary_range !== 'يحدد بعد المقابلة' && (
                                            <>
                                                <span className="text-slate-300">•</span>
                                                <span className="text-amber-600 font-bold">{job.salary_range}</span>
                                            </>
                                        )}
                                    </p>
                                </div>
                        ...
                        <div className="prose prose-lg max-w-none text-slate-600 mb-12 bg-slate-50/50 p-8 rounded-[2rem] border border-slate-100">
                            <h3 className="text-slate-900 font-black mb-4">تفاصيل الوظيفة والشروط:</h3>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            {(job.contact_phone || job.phone_number || job.contact_email) && (
                                <div className="relative">
                                    <button onClick={() => setShowContactOptions(!showContactOptions)} className="w-full flex items-center justify-center gap-3 bg-[#115d9a] text-white py-4 rounded-2xl font-black shadow-lg hover:bg-[#0e4d82] transition-all">
                                        <Phone className="w-6 h-6" /> تواصل مع جهة التوظيف <ChevronDown className={`w-5 h-5 transition-transform ${showContactOptions ? 'rotate-180' : ''}`} />
                                    </button>
                            ...
                            {job.contact_email && (
                                <a href={`mailto:${job.contact_email}`} className="flex items-center justify-center gap-3 bg-orange-50 text-orange-600 py-4 rounded-2xl font-black border border-orange-100 hover:bg-orange-100 transition-all shadow-sm">
                                    <Mail className="w-6 h-6" /> إرسال بريد إلكتروني
                                </a>
                            )}
                            ...
                            {(job.application_link || job.source_url) && (
                                <div className="md:col-span-2">
                                    <a href={job.application_link || job.source_url} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-3 bg-gradient-to-r from-emerald-600 to-teal-600 text-white py-4 px-8 rounded-2xl font-black hover:from-emerald-500 hover:to-teal-500 transition-all shadow-xl hover:shadow-emerald-500/25 w-full text-lg">
                                        <Globe className="w-6 h-6 text-white" /> التقديم المباشر على الوظيفة (الرابط الرسمي)
                                    </a>
                                </div>
                            )}
                        </div>
